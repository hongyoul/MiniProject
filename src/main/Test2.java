...
class Order2 {
   ...
   static LocalDateTime curDateTime = LocalDateTime.now();
   static DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
   static String formatDate = curDateTime.format(formatter);

   // 생성자
   public Order2(String name, String productName, int quantity, int price, String formatDate) {
      super();
      this.name = name;
...
// 상문 주문 클래스
class OrderStart extends Order2{
   
   public OrderStart() {
		super(name, productName, quantity, price, formatDate);
   }
		

// 상품 주문하는 함수
   public static void nextOrder(PrintWriter pw ,Scanner scanner, ArrayList<Order2> list) throws IOException  {
      ...
      list.add(new Order2(name, productName, quantity, price, formatDate));
      
      String str = String.format("주문번호: %d, 고객명: %s, 제품명: %s, 제품의 수량: %d, 제품의 가격: %d, 주문일시 : %s", Test2.count, name, productName, quantity, price, formatDate);
